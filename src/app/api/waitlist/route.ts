  ...
  try {
    ...
    if (process.env.BREVO_API_KEY && !process.env.BREVO_API_KEY.includes('your_')) {
      ...
      await client.transactionalEmails.sendTransacEmail({
        subject: "You're on the list — Welcome to Gitpitch",
        htmlContent: `<div style="font-family: sans-serif; white-space: pre-wrap;">Hi there,
...
The Gitpitch Team</div>`,
        sender: { name: "Gitpitch", email: "hello@gitpitch.demo" },
        to: [{ email }],
      })
    } else {
