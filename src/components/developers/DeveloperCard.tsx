...
export function DeveloperCard({ 
  ...
  const handleShortlistClick = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!onShortlist || isShortlisted || isProcessing) return;
    ...
    <div className="card p-5 flex flex-col h-full bg-[#111110]">
      <div className="flex gap-4 items-start mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {profile.city || (profile.location && profile.location !== 'undefined') ? (
          ...
          >
           ...
           {customAction ? (
            ...
            <button 
              ...
              disabled={isShortlisted || isProcessing}
              className={clsx(
                "flex-1 !py-2 !text-xs !px-0 flex items-center justify-center transition-all",
                isShortlisted 
                  ? "bg-[#c8f060]/10 text-[#c8f060] border border-[#c8f060]/20 cursor-default" 
                  : "btn-primary bg-white text-black border border-transparent hover:bg-white/90 disabled:opacity-70 disabled:cursor-wait"
              )}
            >
              {isProcessing ? "Saving..." : isShortlisted ? "Shortlisted ✓" : "Shortlist"}
            </button>
          )}
        </div>
      </div>
